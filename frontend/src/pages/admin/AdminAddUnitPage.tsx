...
import { Loader2, ArrowLeft, BookOpen, Layers, Info } from 'lucide-react';
...
const AdminAddUnitPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchCourse = async () => {
      ...
      try {
        ...
        setFormData(prev => ({
          ...prev,
          order: (courseData.units_count || 0).toString()
        }));
      } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/courses/units/`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${accessToken}`,
          'Content-Type': 'application/json',
    ...
    <div className="container mx-auto p-4">
      ...
      <div className="mb-6 p-4 bg-blue-50 dark:bg-blue-950/20 rounded-lg border border-blue-200 dark:border-blue-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-semibold text-blue-900 dark:text-blue-100">Adding Unit to:</h2>
            <p className="text-blue-700 dark:text-blue-300 font-medium">{course.name}</p>
            <p className="text-sm text-blue-600 dark:text-blue-400">
              Current units: {course.units_count || 0}
            </p>
          </div>
        </div>
      </div>
      ...
      <Card className="max-w-2xl mx-auto">
        <CardHeader>
          ...
          <p className="text-muted-foreground">
            Add a new unit to organize sections and lessons within this course.
          </p>
        </CardHeader>
              ...
              <div className="flex items-start gap-2">
                ...
                <div>
                  ...
                  <p className="text-sm text-blue-700 dark:text-blue-300">
                    After creating this unit, you'll be able to add sections to organize your lessons and practice exercises.
                  </p>
                </div>
              </div>
            </div>
...
export default AdminAddUnitPage; 